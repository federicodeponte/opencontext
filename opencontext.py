...
# Import from core module (properly maintained GeminiClient)
try:
    from core.gemini_client import GeminiClient
    from core.config import ServiceType
except ImportError:
    # Fallback: try relative import for context-specific client
    try:
        # Add parent to path for shared imports
        _parent = Path(__file__).parent.parent
        if str(_parent) not in sys.path:
            sys.path.insert(0, str(_parent))
        from services.context.shared.gemini_client import GeminiClient
        ServiceType = None
    except ImportError:
